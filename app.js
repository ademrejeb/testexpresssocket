...
const{addpartiesocket,getbyidsocket,attaqueSocket,contreattaqueSocket}=require('./controller/partieController')
mongo
  .connect(config.url, {
...
io.on("connection", (socket) => {
  ...
  const dataofAttaque= await attaqueSocket(data)
  io.emit("attaque1",dataofAttaque )  
  } );

  socket.on("attaquecontre", async (data) =>{
    const dataofAttaque= await contreattaqueSocket(data)
    io.emit("attaque2",dataofAttaque )  
    } );
